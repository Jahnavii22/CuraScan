...
router.post("/:reportId", async (req, res) => {
  try {
    ...
    const filePath =
      report.storagePath ||
      path.resolve(process.env.UPLOAD_DIR || "./uploads", report.filename);
    ...
    const extractedText =
      extracted.extracted_text || extracted.extractedText || "";
    if (!extractedText)
      return res.status(500).json({ ok: false, msg: "Extraction failed" });

    // Step 2️⃣ Parse
    const tmpExtracted = path.join(
      path.dirname(filePath),
      `${Date.now()}_extracted.json`
    );
    fs.writeFileSync(
      tmpExtracted,
    ...
    const tmpParsed = path.join(
      path.dirname(filePath),
      `${Date.now()}_items.json`
    );
    ...
    if (parseOut.error || parseOut.status !== 0) {
      ...
      return res
        .status(500)
        .json({ ok: false, msg: "Parsing failed", error: parseOut.stderr });
    }
    ...
    if (predict.error || predict.status !== 0) {
      ...
      return res
        .status(500)
        .json({ ok: false, msg: "Prediction failed", error: predict.stderr });
    }
    ...
    try {
      const recommendInput = {
        mlPredictions: finalPreds,
        extractedValues: items,
        reportId,
      };

      recommendations = await runRecommendations(
        PYTHON_PATH,
        recScript,
        recommendInput,
        {
    ...
    try {
      fs.unlinkSync(tmpExtracted);
      fs.unlinkSync(tmpParsed);
    } catch {}
    ...
    res
      .status(500)
      .json({ ok: false, msg: err.message, stack: err.stack });
  }
});
