...
def main():
    parser = argparse.ArgumentParser(
        description="Simple PDF text extractor with optional Groq summarization."
    )
    parser.add_argument(
        "pdf_path",
        nargs="?",
        default=r"C:\Users\jahna\Internship-BAA\CuraScan\backend\pdf\BR100012.pdf",
        help="Path to the PDF file (optional)",
    )
    parser.add_argument(
        "--summarize",
        action="store_true",
        help="Send extracted text to Groq for a short summary (requires GROQ_API_KEY in .env)",
    )
    ...
    if args.summarize:
        ...
        if not GROQ_API_KEY:
            ...
            print("[WARN] groq SDK not installed. Run: pip install groq")
        else:
    ...
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(
            {"file": pdf_path, "extracted_text": extracted},
            f,
            ensure_ascii=False,
            indent=2,
        )
